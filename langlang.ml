...
let rec eval env exp stdinBuff =
	...
	| Conc (Language x, String y) -> (env,  Conc (x, Language y), stdinBuff)
	| Conc (String x, Language y) -> (env,  Conc (Language x, y), stdinBuff)
	| Conc (String x, String y) -> (env,  Conc (Language x, Language y), stdinBuff)
